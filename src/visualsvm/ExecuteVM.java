...
public class ExecuteVM {
    ...
    public ExecuteVM(int[] code, int[] sourceMap, List<String> source) {
        ...
        for (var line : this.source) {
            ...
            if (macro.length > 1) {
                if (printArgumentLineNumber) {
                    codeLines.add(
                            CodeLine.lineWithBreakpoint(
                                    String.format(
                                            "%5d: %s   | %5d: %s",
                                            realIp++, macro[0], realIp++, macro[1])));
                } else {
                    codeLines.add(
                            CodeLine.lineWithBreakpoint(String.format("%5d: %s", realIp++, line)));
                    realIp++;
                }
            } else {
                codeLines.add(
        ...
        this.memPanel =
                new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, this.stackScroll, this.heapScroll);
        ...
        this.outputText.addKeyListener(
                new KeyListener() {
                    @Override
    ...
    private void setMem() {
        ...
        //                IntStream.range(0, MEMSIZE).mapToObj(x -> String.format("%5d: %s", x, x <=
        // hp || x >= sp ? this.memory[x] : ""))
        ...
        this.stackScroll
                .getVerticalScrollBar()
                .setValue(
                        computeScrollDestination(this.stackScroll.getVerticalScrollBar(), this.sp));
        ...
        this.heapScroll
                .getVerticalScrollBar()
                .setValue(
                        computeScrollDestination(this.heapScroll.getVerticalScrollBar(), this.hp));
    }

    private int computeScrollDestination(JScrollBar scroll, int pointer) {
        return Math.max(pointer * (scroll.getMaximum() / MEMSIZE) - scroll.getHeight() / 2, 0);
    }
    ...
    private void update() {
        ...
        int dest =
                this.sourceMap[this.ip] * s.getMaximum() / this.codeLineCount - s.getHeight() / 2;
